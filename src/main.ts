...
async function main() {
   ...
   *   = 三口周期循环，方向随队列浮动），沿带流动 3 格消失——轮询次序肉眼可辨。
  ...
  const demoT210 = async (): Promise<string> => {
    ...
    // 轮转: 晶体外壳每 2 秒出现在**下一条**带首（带按创建序起步，成功者移队尾 →
    // 三口周期循环，方向随队列状态浮动），沿 3 格带流动走进顶部存货口。T2.21 起
    // 节流是**设备级**每 40 Tick 至多 1 件（2026-09-05 用户拍板: 多带只分摊不提速）
    // ——轮询次序肉眼可辨（用户 2026-08-25 反馈的"两端口同 Tick 齐出"由此根除）。
    ...
    // 从首件出货起采样 6 件。T2.21 队列语义（成功者移队尾 + 满带跳过后按创建序
    // 重追加）下稳态是周期 3 循环，但起点/方向随队列状态浮动（[2,1,0,2,1,0] 与
    // [0,1,2,0,1,2] 同样合法——带首腾位与节拍计时器的相位竞争会让轮转方向翻转）。
    // 断言只验"6 件内三口都被轮到"（不钉死算术序，同场景A 2026-09-02 修订精神）。
    ...
    return 'T2.10 一键测试完成（场景A 按先到排名序补货 3 口各两次；场景B 出货三口循环（方向随队列浮动）；场景C 跳过中带 + 恢复 左→右→中）';
  };
