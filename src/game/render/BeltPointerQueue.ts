...
//     （与物品排队同构），不穿过 1（旧"接触即消失"alpha 规则退役: 队列化后
//     0 根本到不了 1 身边）。
//   - 击杀: 物品**停稳**（delta=0）且与 0 距离 ≤ CONTACT_KILL_DIST → 该 0 移除
//     （§十三②"1 停稳后 0 才消失"；流动的 1 从 0 身上碾过时 0 在其下方继续
//     存在——zIndex 指针 0.4 < 物品 0.5 自然遮挡）。击杀消耗的 0 由补充规则回填。
//   - 循环: 0 滑出链尾余量 → 从链首带缘再进来（re-entry 槽与链首侧全部实体
//     保持 ≥1 格、≤ 链首边缘——链首被占时在带外等待，遮罩外不可见）。
//   - 补充: 数量 < 链长+1 时在最末实体后方一格回补（该槽必在格网上）——
//     覆盖率（每空格一支 0）不随停走消耗衰减。
//
// 相位对齐（图1 根治）: 每次注入事件（物品数增加的帧，由渲染器判定）全体 0 按
// **最短模距离**（≤半格）平移到新物品（=最末物品）格网——刚性平移不改变 0 之间的
// 间距；正常流动下新物品与既有队列同相位（出货节拍 40 Tick = 整数格 + BeltSystem
// 1/40 栅格对齐），δ=0 零开销；仅停走恢复后新注入与旧队列相位分裂时实际平移，
// 旧相位物品随吸收排出、对齐自我恢复。链哈希偏移（2026-08-25"不同链不锁步"拍板）
// 随 chainVirt 一并退役——同源设备的各带物品类恒相同（注入全在设备 40-Tick 节拍
// 网格上），跨带图案对齐（0-1-0/1-0-1 棋盘）随之成立。
//
...
export class ChainPointerQueue {
   ...
   * 格网上每格一支（≈len+1 支，与旧连续流同密度）。classFrac 取全局相位小数——
   * 与未来物品相位无关属预期（激活重相位一次性兜住）。
   */
  seed(chainLen: number, classFrac: number): void {
    ...
    for (let n = nMin; n <= nMax; n++) {
      this.arrows.push({ id: this.nextId++, pos: n + classFrac });
    }
  }
  ...
  advance(chainLen: number, steps: number, items: readonly QueueItemRef[]): void {
    ...
    // 1. 击杀: 停稳物品接触范围内的指针移除（"1 停稳后 0 才消失"）
    if (items.length > 0) {
      const stopped = items.filter((it) => it.stopped);
      for (let i = this.arrows.length - 1; i >= 0 && stopped.length > 0; i--) {
  ...
  private replenish(chainLen: number, items: readonly QueueItemRef[]): void {
    ...
    while (this.arrows.length < target) {
      ...
      this.arrows.push({ id: this.nextId++, pos: min === Infinity ? 0 : min - 1 });
    }
  }
}
