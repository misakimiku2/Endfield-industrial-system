...
- **满带判定用严格 >**：注入点与入口最近物品间距必须**严格大于** MIN_ITEM_GAP（BeltSystem 导出，与跨段入口空位判定同源）。等号边界依赖 beltPhase 恰取 0.0（浮点累加 0.025 不保证命中），严格不等使"断头段最多堆 2 件(0.5/0.25)、第 3 件留在输出槽"成为确定性行为（满带 → 返回 null 留槽，每 Tick 重试，带腾位即恢复，对称 T2.6 疏通）。
...
- **回归**：t23/t24/t25/t26 单测（40/21/45/39）与浏览器验收（t25 18/18、t26 14/14、t23-t24 9/9）无破坏。
