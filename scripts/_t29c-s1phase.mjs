...
for (const s of [1, 2, 3]) {
  ...
  for (const b of belts) {
    ...
    console.log(`s1-${s} belt(x=${b.gx}): ${ys.length} 支 @ ${ys.map((y) => y.toFixed(1)).join(', ')} | 相位 mod1: [${phases}]`);
  }
  ...
  if (s > 1) {
    for (let b = 0; b < 3; b++) {
      ...
      console.log(`  s1-${s - 1}→s1-${s} belt${b + 1} 最近匹配残差: ${ds.map((d) => d.toFixed(1)).join(', ')}（≤2.5px 视为同指针随拍移动）`);
    }
  }
}
// 三带互相位差（同一拍）
...
for (const s of [1, 2, 3]) {
  ...
  console.log(`s1-${s}: |belt1−belt2|=${diff(a, b).toFixed(3)}  |belt1−belt3|=${diff(a, c).toFixed(3)}  |belt2−belt3|=${diff(b, c).toFixed(3)}（0=完全同相）`);
}
