...
> 仓库取货口/存货口——无限源/汇、非生产设备、Depot 整图+单层 LOGO+Status 悬停高亮、R 四档朝向（T2.17 修订: 90°/270° 占地宽高互换 3×1↔1×3）、
> 仅中间格是端口（T2.18 修订: 侧格不能起带/对接，吞吐 1件/2秒/设备）、接带面统一朝上（T2.19 修订: 存货口与取货口一致从上方接入，180°=朝下）、端口残段水平镜像修正（T2.20: 残段恒落在靠带一侧、保持设备之下钻入观感）；
