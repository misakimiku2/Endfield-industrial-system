...
> **📝 实现笔记已归档（2026-08-27）**: 各已完成任务的「实现笔记」已从 implementation-phase-2.md
> 抽出至 [实现笔记/](实现笔记/) 目录（T2.1~T2.10、T2.12 共 11 份，原文逐字保留）。
> 计划文档现在只含任务目标/需求/验收标准 + 归档链接——发给 AI 做其中某个任务时不再
> 携带无关任务的历史实现细节。注意: **各笔记中的部分结论已被后续任务修订**（如 T2.1 的
> "物品=实体 pointer"约定已于 T2.10 期间退役、指针显隐方案最终定稿为 v11 箭头相位跟随
> 本链领头物品），按时间线阅读时以新任务的修订记录为准。
...
> 然后 读 `doc/implementation-phase-2.md` 中 **T2.8 章节**（一格一物品模型修订、设备弹窗参考旧
> Flutter 项目的决定在 T2.6/T2.7，历史实现细节归档于 `doc/实现笔记/` 目录）。读完后我们从 T2.8 开始。
...
| 实现笔记/（目录） | — | 🟢 P2 各任务 | 各已完成任务的实现笔记归档（T2.1~T2.10、T2.12 共 11 份，2026-08-27 自计划文档抽出）。按需查阅，做新任务不必读 |
