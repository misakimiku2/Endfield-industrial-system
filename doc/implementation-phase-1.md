...
- **选中框渲染**: 屏幕空间 `Graphics` 挂 `overlayLayer`（zIndex −10，工具栏之下）；每帧用 `worldToScreen` 求 footprint 四角 → **黄色半透明填充（0xffd500, alpha 0.25）+ 纯白矩形描边**（用户反馈浅灰地面上白线一眼难锁定，填充层让选中设备整体染黄、一目了然；描边要求无黑边）；线宽随 zoom **等比缩放**——zoom=1 基准 4px、zoom=4 时 16px、zoom=0.25 时下限 1.5px，相对设备的视觉比例恒定（用户反馈"缩小太明显/放大太细"的修正）；顶点像素取整避免半像素发糊；视图旋转下画旋转四边形，跟随相机平移/缩放/旋转过渡动画（worldToScreen 用 displayRotation）
...
- **选中框高亮可调项**：填充色/透明度（`SELECTION_FILL_COLOR` / `SELECTION_FILL_ALPHA`）与线宽基准/下限（`SELECTION_LINE_WIDTH_AT_ZOOM_1` / `SELECTION_LINE_WIDTH_MIN`）集中在 SelectionSystem 常量；若黄色太抢眼可降低 alpha，或换蓝/绿色系（与放置预览蓝、不可放橙红区分）
...
- **一键 100 设备**: `__game.spawnBenchmarkDevices(100)` 走**真实放置路径**
  （`placeAt` → BuildingComp + OccupancyMap + RenderSystem），100 个 refining_unit
  **随机打乱全部候选锚点后贪心放置**（与随机分布等价，但零失败重试）；
  **每次执行先清空旧设备再生成**（重置式语义，重复执行不会叠加刷屏）；
...
设备底座改为九宫格切件（角/边/中 9 块小图）运行时按 footprint 平铺拼装，图集面积与设备尺寸解耦：一套件服务任意 n×m 设备（w,h ≥ 2），新增尺寸零美术成本，devices 图集回落 4096²。

### 背景

T2.8 期间 devices 图集 37 帧顶满 4096² → 临时扩容 8192（asset-manifest MAX_ATLAS_SIZE）。但层帧全画布占位模式下面积 ∝ n²：一台 6×6 设备全套层帧 ~45M 像素吃掉 8192² 的 2/3。经与 3x3_unit.svg 实际结构核对（base 层本就按 `base-pos-{r}-{c}` 分块绘制），用户提出的九宫格平铺方案成立且为最优路线。

### 需求

四个子任务（详细设计见 S2 §7）：

- **T1.11a 素材规范与源文件**: 绘制 `nineslice_unit.svg`（3×3 画布、9 个 `slice-*` 切片组、平铺连续性硬约束）；修订 S1 增补"九宫格设备"章节
- **T1.11b 打包管线**: pack-assets 切片提取（9 帧 `nineslice/*`）+ 层帧白名单（只留 logo/port-*/arrow-*，砍无人消费的整层帧）+ equipment/logo trim（spriteSourceSize + trimOffset 元数据）；图集回落 4096²
- **T1.11c 拼装渲染器**: `NineSliceAssembler`（按 w×h 平铺 9 件成容器，与设备 Sprite 同旋转数学）+ RenderSystem/PlacementSystem 分支接线 + `BuildingDefinition.baseStyle` 字段（缺省 'whole' 向后兼容）
- **T1.11d 试点迁移与回归**: refining_unit 迁移九宫格（3×3 = 最小完整形态，像素级对比验收）+ 6×3/5×5/6×6 demo + t21~t28 全量回归 + 性能基准

### 验收标准（你在浏览器看到的效果）

- **精炼炉外观不变**: 迁移后同视角同缩放（zoom 1/2/4）截图与迁移前逐像素一致
- **任意尺寸 demo**: 6×3、5×5、6×6 测试设备放置——边框完整、柱子钉在每条内部格线端部、无平铺接缝，旋转 90/180/270° 正确
- **图集瘦身**: `npm run pack-assets` 输出回到 4096²
- **性能**: 100 台 3×3 + 50 台 6×6 九宫格设备 FPS ≥ 55
- **零回归**: t21~t28 全部测试通过；whole 路径设备渲染不变
...
解决 S2 §11.4 端口并入切片后的"顶/底行每格一口"假设：把端口内容拆为独立切片组
——固体口 `port-*`（顶/底行 6 组）+ 液体口 `lport-*`（四边 8 组，含未来液体口
上/下边的场景）——运行时按 `BuildingDefinition.ports` 派生的**四边端口类型掩码**
（顶/底逐格 + 左/右逐行 × 固体/液体位）逐位叠加。部分端口（采种机式）、整行/完全
无端口、液体口任意边/任意行设备全部零端口美术成本。

### 需求（详细设计见 S3 §8）

- **a 美术-固体**: slice 组端口三元素剪切到 `port-*` 组（坐标零修改）
- **b 美术-液体**: `lport-l/r` 现素材归组 + `lport-t*/b*` 顶/底新绘（半圆盘贴横边带）
- **c 管线**: `extractNinesliceSlices` 识别 `port-*`/`lport-*` → `nineslice/*` +14 帧
- **d 运行时**: `portMaskFromDef`（含旧 PortType 过渡规则）+ `buildNineSlicePorts` + 烘焙缓存键加掩码
- **e 验收**: 见下

### 验收标准（你在浏览器看到的效果）

- **精炼炉外观不变**（固体全掩码 + 左右液体位 = 现状，像素级一致）
- **无端口 demo**: 底座无面板无箭头
- **部分端口 demo**: 如 4×3 掩码 `0101` → 仅两格有端口；旋转后位置正确
- **液体口 demo**: 顶/底液体口贴边正确；5×5 左边两行各一个侧液口
- **零回归**: verify-t1.11 / t28 全绿

### 前置依赖与建议排期

**⚠️ 顶/底液体口需先做 A3 端口数据模型拆分**（`PortType: 'input'|'output'|'liquid'`
→ 方向 × 介质二维，否则顶边液体口无法表达出入口/出口方向；独立小任务，波及
buildings.ts/MachineSystem/PortStatusOps）。固体变体与侧液口无此依赖，可先行。

第一台**部分端口/无端口/液体上下面**设备美术立项时（或之前任何时候——改动小、
现有设备零影响）。预估 1~2 次会话（固体部分 1 次，液体部分 +A3 拆分 1 次）。
