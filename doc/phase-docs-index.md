...
> **进度（2026-08-17）**: T2.0~T2.7 已完成 ✅（传送带创建/物品移动/跨段堵塞/配方/缓冲区/生产计时/输入对接/输出对接；
> 传送带已按用户澄清改为**一格一物品**模型，A9 §2.3/§3 有修订注）。当前任务 **T2.8 设备状态机与终末地风格状态视觉**。
> 后续: T2.9 产线 UI → T2.10 端口轮询 → T2.11 转角带 → T2.12 取/存货口 → T2.13 打通验证 → T2.14 设备移动 → T2.15 设备弹窗（新增）。
...
| **nineslice-port-variant.md** | **S3** | **🔵 P1 补充（T1.12，📋 方案就绪待实施）** | **九宫格端口变体方案：端口拆独立 port-*(固体)+lport-*(液体,四边) 切片组，def.ports 派生四边类型掩码逐位叠加，部分/无端口/液体上下面设备零美术成本（2026-08-20 设计完成，顶/底液体口前置 A3 端口模型方向×介质拆分）** |
