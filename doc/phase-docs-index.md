...
> **进度（2026-09-09 修订）**: **已完成 28 张卡** —— T2.0~T2.12、T2.15、T2.16~T2.29。
> - **T2.16 终点对接辅助**（2026-09-02）+ 同日玩家三轮实测驱动的四项产线语义修订（先到排名轮询 / 进料提速 / 红色真堵分类 / 物品渲染内插）。
> - **T2.17~T2.20**（2026-09-04 仓库口四连修）: 四向旋转（3×1↔1×3 占地宽高互换）/ 单端口化（仅中间格是端口，侧格不能起带对接）/ 接带面统一朝上（与取货口一致）/ 端口残段水平镜像修正。
> - **T2.21~T2.29**（2026-09-05~09-09，**计划外攻坚九张卡**）: 输出端按**创建顺序**轮询、总吞吐恒 1 件/40 Tick（T2.21）→ 指针相位连续性（T2.22）→ 网格对齐与槽位注入（T2.24）→ 输出/指针诊断日志（T2.25）→ 跨段落点保续（T2.26）→ **指针渲染架构重写①**连续箭头流（T2.27）→ **重写②**0/1 统一模型（T2.28，指针=带面自带循环图案）→ 指针队列物理与轮询棋盘（T2.29，补记 a~h 八轮复测，最新一轮 2026-09-09 堵塞语义回改为工作区未提交状态）。
>   ⚠️ 指针方案在 v11 定稿后又被推倒重写两次，最终形态是 **T2.28「0/1 统一模型」+ T2.29「指针队列物理」**，早期笔记的指针结论一律作废。
> - **T2.15 设备弹窗**（2026-09-09）: DOM overlay 弹窗（样式复刻旧 Flutter 项目）——电源开关（暂停正式入口）/信息栏/生产状态摘要（**吸收移除 T2.9b 临时读数**）/删除按钮/取货口产出物品选择（`depotOutputItemId` 逐台可配）。选中即弹窗。
> - **未完成只剩 2 张**: **T2.13 打通验证 / T2.14 设备移动**（T2.14 的 T1.8 长按结构与 OccupancyMap 基建已现成）。
>
> **下一步（建议顺序）**: T2.13（`t213` 一键场景 + `perf_50` 性能基准 + 玩家手动搭建关卡，顺带实机复测 T2.29 指针）→ T2.14 设备移动（落地后在弹窗动作行补「移动」按钮）。
> T2.11 转角带主体已在 T2.0~T2.8 期间提前实现，剩余 4 方向正式验收并入 T2.13 回归。
> 详见 implementation-phase-2.md 文首「进度与执行顺序（2026-09-08 更新）」与「当前状态速览」两节。

> **📝 实现笔记已归档（2026-08-27 首抽，2026-09-08 更新）**: 各已完成任务的「实现笔记」已从 implementation-phase-2.md
> 抽出至 [实现笔记/](实现笔记/) 目录（T2.0~T2.12、T2.16~T2.29 共 25 份，原文逐字保留）。
> 计划文档现在只含任务目标/需求/验收标准 + 归档链接——发给 AI 做其中某个任务时不再
> 携带无关任务的历史实现细节。注意: **各笔记中的部分结论已被后续任务多次修订**（如 T2.1 的
> "物品=实体 pointer"约定已于 T2.10 期间退役；指针显隐方案 v11 定稿后又在 T2.27/T2.28 **两次推倒重写**，
> 最终形态为 T2.28「0/1 统一模型」+ T2.29「指针队列物理」），按时间线阅读时以新任务的修订记录为准。
...
把下面这段话发给 AI（**先读进度，再决定做哪张卡**）：

> 我要继续 Phase 2 开发。请先读 `doc/phase-docs-index.md` 的"🟢 Phase 2 开发前要读的文档"章节（含 2026-09-08 进度注），
> 然后读 `doc/implementation-phase-2.md` 文首的「进度与执行顺序（2026-09-08 更新）」+「当前状态速览」两节，
> 确认当前该做 T2.13 / T2.14 中的哪一张，再读对应任务卡（卡内已附 2026-09-08 现状核实）。
> 历史实现细节归档于 `doc/实现笔记/` 目录（指针相关以 T2.28/T2.29 为准，早期笔记已作废）。
...
| 实现笔记/（目录） | — | 🟢 P2 各任务 | 各已完成任务的实现笔记归档（T2.0~T2.12、T2.16~T2.29 共 25 份，2026-08-27 自计划文档抽出，2026-09-08 增至 25 份）。按需查阅，做新任务不必读 |
