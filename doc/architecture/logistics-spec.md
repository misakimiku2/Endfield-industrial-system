...
- 每个 Cell（物理段）独立维护自己的 `items[]` 队列，每个物品各自追踪 `progress`。
...
**所有传送带必须从设备的输出端口（或仓库取货口）发起创建**。玩家不能在空地上凭空放置独立传送带。

```
[设备输出端口] ──→ 玩家点击此格 ──→ 进入"传送带绘制模式" ──→ 延申/截断
```
...
| 起点 | 说明 |
|------|------|
| 设备 Output Port 所在 Cell | 所有生产设备的输出端口、仓库取货口（Depot Unloader） |
| 已有传送带的末端 Cell（断头传送带） | 延申创建，见 §6.3 |
| 已有传送带的中间 Cell（非末端） | 截断创建，见 §6.4 |
...
- 延申创建 → 新段继承原链 `chainId`，`segmentIndex` 接续递增
- 截断创建 → 原链在截断点分裂为两条独立链（下游链重新分配 `chainId`，见 §6.4）
- 合并创建 → 把一条孤立链接入另一条链的末端，合并 `chainId`（见 §6.4.1）
...
| 创建类型 | 允许的方向 | 禁止的方向 |
|---------|-----------|-----------|
| **延申创建** | 沿原方向继续；或垂直于原方向（90° 转弯） | 反向（逆流方向） |
| **截断创建** | 仅垂直于原链方向 | 原方向（顺流/逆流均禁止，因为原链已占用） |
...
2. 若重叠 → 创建失败，提示玩家"传送带不能穿过设备"。
3. **例外——物流桥（Belt Bridge）**：物流桥是一个 1×1 设备，其设计目的就是**让两条传送带在同一 Cell 交叉运输互不干扰**。因此传送带可以穿过物流桥所在的 Cell（物流桥是合法的交叉点）。详见 §8.1.1。
...
```
1. 玩家选中一个设备（点击设备实体）
2. 设备的输出端口高亮显示（白色/黄色描边）
3. 玩家点击某个输出端口 Cell → 进入"传送带绘制模式"
   - 鼠标移动时显示半透明预览段（吸附网格）
   - 预览段根据鼠标方向自动判断朝向
...
- **删除单段**：仅移除指定段的 Entity。若该段位于链中间，链分裂为两条独立链（同 §6.4 截断逻辑，但不创建新分支）。
- **删除整链**：移除该 `chainId` 的所有段。物品全部消失。
- 删除后，上游链末端变为断头传送带（已有物品继续流到末端后停留）。
...
> **Phase 归属**：物流桥的完整交叉逻辑属于 **Phase 3**。Phase 2 先实现基础传送带（直带 + 转角），物流桥作为"防穿模的合法交叉点"可以先放行创建，但两条链的独立物品流计算可后置。
...
管道系统使用与固体物流**相同的 ECS Component**（`LogisticsComp`），同一套 System（BeltSystem 扩展），仅通过 `category: 'pipe'` 区分行为。Phase 3+ 实现时：
...
| 同段多物品 | 一段传送带上可同时存在多个不同类型物品，按最小间距排队（见 §2.3） |
| 两种停止点 | 端口吸入停在 progress=0.5（格子中心）；堵塞停在 progress=0.99（段尾中心），见 §3 |
| 堵塞逆流传播 | 下游满 → 首物品停段尾 → 后方被间距夹住 → 逆流向上游传播 |
...
| Phase 1 先实现直线传送带 | 转角留到 Phase 2；分流器/汇流器/物流桥/物品准入口留到 Phase 3 |
| 管道系统 Phase 3+ | 共享拓扑模型，参数不同（2.0 格/秒、容量 4） |
