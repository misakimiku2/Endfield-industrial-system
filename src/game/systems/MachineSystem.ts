...
//       队首物品停在门口(STOP_MAX=0.5)时吸入输入槽（A9 §3.3/§3.6，
...
export class MachineSystem implements SimulationSystem {
   ...
   * 传送带段（A9 §6.7），尝试吸入停在门口(0.5)的队首物品；槽满/类型不符则物品
   * 留在传送带上（每 Tick 重试，槽腾出即吸入，A9 §3.5）。每端口每 Tick 至多 1 件。
   */
  private absorbBeltInputs(
    ...
    for (const cell of inputPortCells(gx, gy, def, comp.direction)) {
      ...
      if (!seg) continue;
      const absorbed = tryAbsorbHeadItem(seg, comp, def.bufferCapacity);
      if (absorbed !== null) {
        this.emit({
          type: 'input', handle,
