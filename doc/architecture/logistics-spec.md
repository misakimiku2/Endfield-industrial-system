...
> **修订（2026-08-17 用户确认，仅记录未实现）**: 吸入触发点应为**设备输入端口格的中心**——物品
> 走到供给格尾（progress 1.0）后行程延伸进端口格半格，到达端口格中心才消失进入输入槽
> （视觉"走进设备"，以 `精炼炉设备说明.md` 为准）；堵塞时物品停在**供给格**（不进设备）。
> 当前实现仍是本节原文（供给格中心 0.5 吸入），修正见 implementation-phase-2.md T2.6 已知遗留。
...
| 两种停止点 | **统一格中心 0.5（2026-08-17 修订）**：端口吸入触发、断头堵塞、满带堆放同点（见 §3） |
