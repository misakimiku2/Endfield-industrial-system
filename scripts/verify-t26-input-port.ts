// T2.6 验证: 传送带 → 设备输入对接（预约制端口格中心吸入: 0.5 判定+槽即占用+1.5 消失）
// 依据: implementation-phase-2.md T2.6（2026-08-17 修订）、精炼炉设备说明.md（吸入点确认）、
//       A9 §3.3(端口吸入)/§3.5(疏通)/§3.6(副作用)/§6.7(连接判定)、
...
//     3. tryAbsorbHeadItem 预约: 空段→null；队首未到门口(0.4)→null；
//        到门口+空槽→预约(槽+1+entering，物品保留)；releaseArrivedItems: <1.5 不移除、≥1.5 移除
//     4. tryAbsorbHeadItem: 槽满→null（物品停门口无 entering）；锁定异类型→null
...
//    13. 每端口每 Tick 至多预约 1 件: 两件物品(0.5/0.5) → 首 Tick 只预约队首，上游件随后到达再预约
//    14. 多端口同 Tick 各预约 1 件: 三条供给带各 1 件在门口 → 同 Tick 全部预约（单输入槽合堆 ×3）
//    15. 跨段流动到门口: 两段链物品从上游段出发 → 跨段 → 到门口预约 → 走进设备消失
...
assertEq(PORT_ENTER_PROGRESS, 0.5, '1e. 预约判定点 = 0.5（供给格中心，与 BeltSystem.STOP_MAX 同源）');
assertEq(PORT_RELEASE_PROGRESS, 1.5, '1f. 放行/移除点 = 1.5（端口格中心，与 BeltSystem.PORT_ENTER_DONE 同源）');
...
console.log('[tryAbsorbHeadItem 预约判定 + releaseArrivedItems 放行]');
{
  ...
  assertEq(tryAbsorbHeadItem(s3, comp, 50), 'originium_ore', '3c. 到门口+空槽 → 预约源矿（返回 itemId）');
  assertEq(s3.items.length, 1, '3c2. 预约后物品保留在段上（entering 标记，待走进设备）');
  assertEq(s3.items[0].entering, true, '3c3. 物品标记 entering=true（BeltSystem 放行推进到 1.5）');
  assertEq(comp.bufferInput[0], { itemId: 'originium_ore', count: 1 },
    '3c4. 预约即槽位占用: 输入槽锁定源矿并 count+1');
  // 放行: entering 物品未到 1.5 不移除；到 1.5 移除
  s3.items[0].progress = 1.4;
  assertEq(releaseArrivedItems(s3), [], '3d. entering 物品未到端口格中心(1.4) → 不移除');
  s3.items[0].progress = 1.5;
  assertEq(releaseArrivedItems(s3), ['originium_ore'], '3e. entering 物品到端口格中心(1.5) → 移除（视觉消失）');
  assertEq(s3.items.length, 0, '3e2. 段上物品消失（完成进入设备）');
  // 非 entering 物品即使 progress ≥1.5 也不被放行（防御）
  const s3f = mk([['originium_ore', 1.5, false]]);
  assertEq(releaseArrivedItems(s3f), [], '3f. 非 entering 物品(异常注入 1.5) → 不被放行');
  ...
  assertEq(s4.items[0].entering ?? false, false, '4a3. 物品未获 entering 标记（停在供给格中心）');
...
const place = (gx: number, gy: number, dir: 0 | 90 | 180 | 270 = 0): BuildingComp => {
  ...
  // 结算暂缓不消耗输入槽——本测试只观察输入对接，排除生产扣料的时序干扰。
  comp.bufferOutput[0] = { itemId: 'origocrust', count: def.bufferCapacity };
...
tick(21); // 0→0.5 需 20 Tick（第 20 Tick 预约），+1 Tick 物品已开始走进设备
assertEq(f1.bufferInput[0], { itemId: 'originium_ore', count: 1 },
  '5a. 物品走到门口 → 同 Tick 预约: 输入槽 源矿 ×1 (已锁定)');
assert(b1.items.length === 1 && b1.items[0].entering === true && b1.items[0].progress > 0.5,
  `5b. 预约后物品保留并走进设备（entering=true, progress=${b1.items[0]?.progress.toFixed(3)} > 0.5）`);
...
tick(2); // 第 1 Tick 预约（槽回 50、entering），第 2 Tick 物品开始走进设备
assertEq(f2.bufferInput[0].count, 50, '8a. 疏通后门口物品立即被预约（槽回 50/50）');
assert(b2.items.length === 1 && b2.items[0].entering === true && b2.items[0].progress > 0.5,
  '8b. 预约物品正在走进设备（entering=true, progress > 0.5）');
tick(41); // 0.5→1.5 需 40 Tick
assertEq(b2.items.length, 0, '8c. 预约物品到端口格中心消失（段上清空）');
...
// 13: 每端口每 Tick 至多预约 1 件（一格一物品: 门口格只容 1 件，上游件依次到达再预约）
...
tick(1);
assertEq(f6.bufferInput[0].count, 1, '13a. 首 Tick 只预约门口段队首 1 件');
assertEq(b7.items.length, 1, '13b. 上游件留在上游段（门口腾空后才开始前进）');
// 时序: 门口件预约后 40 Tick 走进设备(1.5)消失 → 上游件跟进到门口(≤20 Tick)预约 → 再 40 Tick 消失
tick(110);
assertEq(b7.items.length, 0, '13c. 上游件跨段到达门口被预约并走进设备（上游段清空）');
assertEq(f6.bufferInput[0].count, 2, '13d. 输入槽 源矿 ×2（同槽合堆）');

// 14: 多端口同 Tick 各预约 1 件（三条供给带 → 三个输入端口）
...
tick(1);
assertEq(f7.bufferInput[0].count, 3, '14. 三条供给带各 1 件在门口 → 同 Tick 全部预约（单输入槽 ×3）');

// 15: 跨段流动到门口（两段链: 物品从上游段段首出发 → 跨段 → 门口预约 → 走进设备消失）
...
// 时序: 1 格(40 Tick) + 半格(20 Tick) + 预约后走进设备(40 Tick) + 裕量
tick(105);
assertEq(f8.bufferInput[0].count, 1, '15. 两段链物品跨段到门口 → 预约进入（跨段 + 端口对接连续路径）');
assertEq(bUp2.items.length, 0, '15b. 上游段已清空（物品顺利移交下游并走进设备）');
