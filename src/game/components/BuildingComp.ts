...
export interface BuildingComp {
   ...
   * 不改定义序）。预约成功或端口跳过（无供给带/类型不符/未到门口）指针都 +1——
   * **沿先到排名序**（见 inputArrivalRank）前进；全部输入槽满载时冻结不动、不重置
   * （A8 §4.1"轮询指针不重置"，精炼炉设备说明"A 成功那么下一次应该直接轮到 B"）。
   * 放置时初始化 0。
   */
  inputPollIndex: number;
  /**
   * 输入端口先到排名 (T2.10 修订，2026-09-02 用户拍板"先到先得")。每口第一次有
   * 物品到达供给格中心(0.5)的仿真 Tick（一次性戳记，之后不再更新）；Infinity = 该口
   * 从未有过物品到门口。轮询走访序 = 按此值升序（先到先服务），未到过的口排最后、
   * 相同值按定义序。懒初始化（MachineSystem 首次走访时建全长 n 的数组）。
   */
  inputArrivalRank?: number[];
  /**
   * 输出轮询队列 (A8 §4.2，T2.10→T2.21 修订，2026-09-05 用户重定语义)。按当前
   * 轮询序排列的**活跃接收传送带** EntityHandle（不再存端口下标——轮询单元是
   * 传送带，按 chainId 创建顺序，多带只分摊不提速）。出货成功的带移到队尾（轮转）；
   * 出货失败的带（无接收带 / 满带一格一物品）移出队列，下一 Tick 队列同步时按
   * 创建序重新追加队尾（堵塞集 = 本 Tick 候选带 − 本队列，派生值不落盘）。
   * 放置时初始化为空数组，首次有货出料时由邻接发现（collectReceiverBelts）
   * 按创建序填入；新接入的带按创建序追加队尾（A 独跑中接入 B → 自动 A-B 轮询）。
   */
  outputPollQueue: number[];
  /**
   * 输出节拍计时器 (T2.21，2026-09-05)。下一次允许出货的仿真 Tick——设备级节拍:
   * 每台设备每 40 Tick（=一格传送带时长 = 单带吞吐上限）至多成功输出 1 件，
   * 与接收带条数无关（1 条带 1-1-1 / 2 条各 1-0-1 / 3 条各 1-0-0）。成功出货后
   * = simTick + 40；无可写带不推进（带腾位即恢复）。undefined = 从未出过货
   * （立即允许）。
   */
