...
> **进度（2026-09-09 修订）**: **已完成 27 张卡** —— T2.0~T2.12、T2.16~T2.29。
...
> - **未完成只剩 3 张**: **T2.13 打通验证 / T2.14 设备移动 / T2.15 设备弹窗**（三张均零开跑，`src/` 内无对应实现；T2.14 的 T1.8 长按结构与 OccupancyMap 基建已现成）。
>
> **下一步（建议顺序）**: T2.13（`t213` 一键场景 + `perf_50` 性能基准 + 玩家手动搭建关卡，顺带实机复测 T2.29 指针）→ T2.15（**先切最小首版: 弹窗外壳 + 电源开关**，让"关闭设备"脱离调试钩子 `__game.setPaused`）→ T2.14 设备移动。
...
> 确认当前该做 T2.13 / T2.15 / T2.14 中的哪一张，再读对应任务卡（卡内已附 2026-09-08 现状核实）。
